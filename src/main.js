...
function populateYearFilter(visits) {
  ...
  sel.onchange = ()=>{ currentYearFilter = sel.value; recomputeVisitedPrefectures(); for (const id of Object.keys(countryLayers)) if (map.hasLayer(countryLayers[id])) { if (id.endsWith('_pref')) countryLayers[id].setStyle(stylePrefectureFeature); else countryLayers[id].setStyle(styleFeature); } updateStatCounts(); };
}
...
function populateCountryFilter(sources) {
  ...
  sel.onchange = ()=>{ currentCountryFilter = sel.value; updateVisibleLayers(); updateStatCounts(); };
...
// Inject 已走过 / 待探索 count placeholders into the legend, right after each label.
// Built entirely from JS so index.html does not need to be modified.
function initStatCounts() {
  ...
  const makeSpan = (id) => {
    ...
    s.id = id;
    s.textContent = '(0)';
    s.style.cssText = 'margin-left:4px;color:#555;white-space:nowrap';
    return s;
  };
  const visitedSpan = makeSpan('visitedCount');
  const unvisitedSpan = makeSpan('unvisitedCount');
  let doneVisited = false, doneUnvisited = false;
  for (const node of Array.from(legend.childNodes)) {
    if (node.nodeType !== 3) continue; // text nodes only
    const t = node.textContent || '';
    if (!doneVisited && t.indexOf('已走过') !== -1) {
      legend.insertBefore(visitedSpan, node.nextSibling);
      doneVisited = true;
    } else if (!doneUnvisited && t.indexOf('待探索') !== -1) {
      legend.insertBefore(unvisitedSpan, node.nextSibling);
      doneUnvisited = true;
    }
    if (doneVisited && doneUnvisited) break;
  }
  // Fallback: append at the end if the labels were not found
  if (!doneVisited) legend.appendChild(visitedSpan);
  if (!doneUnvisited) legend.appendChild(unvisitedSpan);
}

// Recompute 已走过 / 待探索 counts for the currently visible layers and refresh the display.
// Japan switches between prefecture (zoom < 7) and municipality layers automatically, so this
// follows the currently displayed layers: prefecture counts use visitedPrefectures, while
// municipality counts use per-feature visit records.
function updateStatCounts() {
  ...
  for (const id of Array.from(displayedCountryIds)) {
    ...
    if (!layer) continue;
    const isPref = id.endsWith('_pref');
    const features = layer.getLayers ? layer.getLayers() : [];
    ...
    for (const l of features) {
      ...
      if (isPref) {
        ...
        if (prefName && visitedPrefectures.has(prefName)) visited++;
      } else {
        const fid = getFeatureId(p);
        const visit = (VISITS_DATA && fid) ? VISITS_DATA[fid] : null;
        if (isVisitedForFilter(visit)) visited++;
      }
    }
  }
  ...
  if (unvisitedEl) unvisitedEl.textContent = '(' + Math.max(0, total - visited) + ')';
}
...
map.on('zoomend', ()=>{ updateVisibleLayers(true); updateStatCounts(); });
