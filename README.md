...
* 左上角控制面板的图例中新增【已走过】与【待探索】的具体数量统计，例如：已走过 (50) 待探索 (300)
* 数量会跟随【年份】与【国家/地区】筛选条件动态更新：
    * 切换年份或国家/地区后，立即重新计算并刷新数量
* 日本的统计自动跟随当前缩放层级：
    * 缩小查看（zoom < 7，展示都道府県）时，按都道府県数量统计
    * 放大查看（zoom >= 7，展示市区町村）时，按市区町村数量统计
* 仅作用于展示页（main.js），不影响本地编辑器
